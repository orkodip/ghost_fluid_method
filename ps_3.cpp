...
void GFM::LS()
{
	double Xc=0.5,Yc=0.5,rad=0.25,Dc;	//circle parameters
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
...
void GFM::f_jump(int f_flag,int i0,int j0,int i1,int j1,double Beta_p,double Beta_m,double *Beta_f,double *F_f)
{
	...
	*Beta_f=Beta_p*Beta_m*(abs(Phi_p)+abs(Phi_m))/(Beta_p*abs(Phi_m)+Beta_m*abs(Phi_p));
	...
	jump_0=-exp(-CX[i0]*CX[i0]-CY[j0]*CY[j0]);	//i0,j0
	jump_1=-exp(-CX[i1]*CX[i1]-CY[j1]*CY[j1]);	//i1,j1
	jump=jump_0*Phi_m+jump_1*(1.0-Phi_m);	//jump in magnitude
	jump_0=8.0*(2.0*CX[i0]*CX[i0]+2.0*CY[j0]*CY[j0]-CX[i0]-CY[j0])*exp(-CX[i0]*CX[i0]-CY[j0]*CY[j0]);
	jump_1=8.0*(2.0*CX[i1]*CX[i1]+2.0*CY[j1]*CY[j1]-CX[i1]-CY[j1])*exp(-CX[i1]*CX[i1]-CY[j1]*CY[j1]);
	if(i0==i1)	//y direction
	{
		dn=abs(Ym[j1]-Ym[j0]);
		norm_0=4.0*CY[j0]-2.0;	//i0,j0
		norm_1=4.0*CY[j1]-2.0;	//i1,j1
	}
	else	//x direction
	{
		dn=abs(Xm[i1]-Xm[i0]);
		norm_0=4.0*CX[i0]-2.0;	//i0,j0
		norm_1=4.0*CX[i1]-2.0;	//i1,j1
	}
...
void GFM::Poisson(int *C,int *R,double *A,double *b)
{
	...
	double Beta_p=1.0,Beta_m=2.0;	//coefficients in the two subdomains
	double TH_l=0.0,TH_r=0.0,TH_t=0.0,TH_b=0.0;	//Dirichlet boundary conditions
	...
	for(int j=1;j<=J;j++)	//create source term
		for(int i=1;i<=I;i++)
			if(Phi[j][i]<0.0) f[j][i]=8.0*(CX[i]*CX[i]+CY[j]*CY[j]-1.0)*exp(-CX[i]*CX[i]-CY[j]*CY[j]);
			else f[j][i]=0.0;
	for(int j=1;j<=J;j++)	//create discreized linear system
	{
		for(int i=1;i<=I;i++)
		{
			...
			{
				b[(j-1)*I+i-1]-=2.0*(A_w*TH_l+A_s*TH_b);
			...
			{
				b[(j-1)*I+i-1]-=2.0*(A_w*TH_l+A_n*TH_t);
				C[cnt]=(j-2)*I+i-1;	//1,J-1 term
				A[cnt]=A_s;
				cnt++;
				C[cnt]=(j-1)*I+i-1;	//1,J term
			...
			{
				b[(j-1)*I+i-1]-=2.0*(A_e*TH_r+A_n*TH_t);
				C[cnt]=(j-2)*I+i-1;	//I,J-1 term
				A[cnt]=A_s;
				cnt++;
				C[cnt]=(j-1)*I+i-2;	//I-1,J term
			...
			{
				b[(j-1)*I+i-1]-=2.0*(A_e*TH_r+A_s*TH_b);
