...
void GFM::LS()
{
	...
	for(int i=1;i<=I;i++)
	{
		p1=0.3-(i-0.5)*dx; p2=(i-0.5)*dx-0.6;
		if(abs(p1)<abs(p2)) Phi[i]=p1;
		else Phi[i]=p2;
	}
...
void GFM::Poisson(int *C,int *R,double *A,double *b)
{
	...
	double Beta_p=1.0,Beta_m=2.0;	//coefficients
	double a_j1=-exp(-0.09),b_j1=1.2*exp(-0.09);	//1st jump conditions
	double a_j2=-exp(-0.36),b_j2=2.4*exp(-0.36);	//2nd jump conditions
	double TH_l=0.0,TH_r=0.0;	//Dirichlet boundary conditions
	double A_e,A_w,F_l,F_r;
	int cnt=0; R[cnt]=0;
	for(int i=1;i<=I;i++)	//create source term
	{
		Phi_m=pow(((i-0.5)*dx),2.0);	//variable is reused
		if(Phi[i]<0.0) f[i]=(8.0*Phi_m-4.0)*exp(-Phi_m);
		else f[i]=0.0;
	}
	for(int i=1;i<=I;i++)	//create discretized linear system
	{
		...
		else
		{
			...
			Phi_m=(i-0.5)*dx;	//x coordinate (variable is reused)
			if(Phi_m<0.5)	//apply 1st jump condition
			{
				if((Phi[i]<=0.0)&&(Phi[i+1]>0.0)) F_r=A_e*(a_j1+b_j1*Phi_p*dx/Beta_p);
				else F_r=-A_e*(a_j1+b_j1*Phi_p*dx/Beta_m);
			}
			else	//apply 2nd jump condition
			{
				if((Phi[i]<=0.0)&&(Phi[i+1]>0.0)) F_r=A_e*(a_j2+b_j2*Phi_p*dx/Beta_p);
				else F_r=-A_e*(a_j2+b_j2*Phi_p*dx/Beta_m);
			}
		}
		if(SGN(Phi[i]*Phi[i-1])==1)
		{
		...
		else
		{
			...
			Phi_p=(i-0.5)*dx;	//x coordinate (variable is reused)
			if(Phi_p<0.5)	//apply 1st jump condition
			{
				if((Phi[i]<=0.0)&&(Phi[i-1]>0.0)) F_l=A_w*(a_j1-b_j1*Phi_m*dx/Beta_p);
				else F_l=-A_w*(a_j1-b_j1*Phi_m*dx/Beta_m);
			}
			else	//apply 2nd jump condition
			{
				if((Phi[i]<=0.0)&&(Phi[i-1]>0.0)) F_l=A_w*(a_j2-b_j2*Phi_m*dx/Beta_p);
				else F_l=-A_w*(a_j2-b_j2*Phi_m*dx/Beta_m);
			}
		}
