...
void GFM::f_jump(int f_flag,int i0,int j0,int i1,int j1,double Beta_p,double Beta_m,double *Beta_f,double *F_f)
{
	...
	*Beta_f=Beta_p*Beta_m*(abs(Phi_p)+abs(Phi_m))/(Beta_p*abs(Phi_m)+Beta_m*abs(Phi_p));
	...
	Phi_p=CX[i0]*CX[i0]+CY[j0]*CY[j0];	//variable is reused
	jump_0=0.1*Phi_p*Phi_p-0.01*log(2.0*sqrt(Phi_p))-Phi_p;	//i0,j0
	Phi_p=CX[i1]*CX[i1]+CY[j1]*CY[j1];	//variable is reused
	jump_1=0.1*Phi_p*Phi_p-0.01*log(2.0*sqrt(Phi_p))-Phi_p;	//i1,j1
	...
	N1=N1.unit();
	Phi_p=CX[i0]*CX[i0]+CY[j0]*CY[j0];	//variable is reused
	jump_0=(4.0*Phi_p-0.1/Phi_p-2.0)*(CX[i0]*N0.x+CY[j0]*N0.y);	//i0,j0
	Phi_p=CX[i1]*CX[i1]+CY[j1]*CY[j1];	//variable is reused
	jump_1=(4.0*Phi_p-0.1/Phi_p-2.0)*(CX[i1]*N1.x+CY[j1]*N1.y);	//i1,j1
...
void GFM::Poisson(int *C,int *R,double *A,double *b)
{
	...
	{
		A_e=CX[1]*CX[1]+CY[j]*CY[j];	//variable is reused
		TH_l[j]=0.1*A_e*A_e-0.01*log(2.0*sqrt(A_e));
		A_e=CX[I]*CX[I]+CY[j]*CY[j];	//variable is reused
		TH_r[j]=0.1*A_e*A_e-0.01*log(2.0*sqrt(A_e));
	}
	for(int i=1;i<=I;i++)	//bottom and top boundary conditions
	{
		A_e=CX[i]*CX[i]+CY[1]*CY[1];	//variable is reused
		TH_b[i]=0.1*A_e*A_e-0.01*log(2.0*sqrt(A_e));
		A_e=CX[i]*CX[i]+CY[J]*CY[J];	//variable is reused
		TH_t[i]=0.1*A_e*A_e-0.01*log(2.0*sqrt(A_e));
	}
	int cnt=0; R[cnt]=0;
	for(int j=1;j<=J;j++)	//create source term
		for(int i=1;i<=I;i++)
			if(Phi[j][i]<0.0) f[j][i]=4.0;
			else f[j][i]=16.0*(CX[i]*CX[i]+CY[j]*CY[j]);
